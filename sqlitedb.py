...
class DB():
    ...
    def saveDataList(self, lista): 
            self.connect()
            [self._saveData(dado) for dado in lista]
            self.close()
    ...
    def findExactMath(self, key, nome): 
            func=str
            try:
                float(nome)
                func=float
    ...
    def getDataList(self, listaDeIds):
            return [self.getData(id) for id in listaDeIds]

    def getDataListWithId(self, listaDeIds):
            return [self.getDataWithId(id) for id in listaDeIds]
    
    def findData(self, key, nome):
            return sorted(self.getDataList(self.findData(key, nome)), key=lambda x: x[key])

    def findBiggerThan(self, key, valor): 
    ...
    def findSmallerThan(self, key, valor):    
        assert type(valor) == int or type(valor) == float, "Entre com valores numéricos"
        self.connect()
        idList=[[list(dado)[0], self.toDict(list(dado)[1:])[key]] 
                        for dado in list(self.cursor.execute("SELECT * FROM "+self.tableName)) 
    ...
    def findListBiggerThan(self,key,valor):
        return sorted(self.getDataList(self.findBiggerThan(key, valor)), key=lambda x: x[key])

    def findListSmallerThan(self,key,valor):
        return sorted(self.getDataList(self.findSmallerThan(key, valor)), key=lambda x: x[key])
   
    def eraseDataById(self, id):
            self.connect()
            id=str(id)
...
def test():
 ...
 "serie":8, "escola":3, "idade":15, "lat":-19.231, "long":47.12331}
     ...
     db.saveDataList([d1,d2,d3]) 
     print("TODOS: \n", db.allData(), "\n\n")
     print("Nomes com ma: \n", db.findData('nome', 'ma'), "\n\n")
     print("Idade 13 \n", db.findData('idade', 13), "\n\n")
     print("Idade maior que 14: \n", db.findListBiggerThan('idade', 14), "\n\n")
     print("Idade menor que 14: \n", db.findListSmallerThan('idade', 14), "\n\n")
     id=db.findData('nome','matheus')[0]
